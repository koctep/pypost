...
## [ ] 2. Закрыть потерю данных в редакторе environment variables

### [ ] 2.1. Ввести единый доменный контракт ключей

- [ ] Дополнить domain-level validation проверкой уникальности нормализованного имени в рамках
  одного environment.
- [ ] Возвращать типизированную причину отказа: empty, invalid syntax или duplicate.
- [ ] Использовать один и тот же validator для добавления, rename, import и программного
  обновления переменных.

### [ ] 2.2. Сделать редактирование транзакционным

- [ ] Не пересобирать `env.variables` из всей визуальной таблицы на каждый `itemChanged`.
- [ ] Хранить редактируемые строки в table model/working copy с устойчивой идентичностью строки.
- [ ] Применять валидное изменение атомарно; при ошибке оставлять исходный ключ и значение.
- [ ] Показывать понятную ошибку при duplicate key и возвращать фокус в проблемную ячейку.
- [ ] Явно определить поведение очистки существующего ключа: подтверждённое удаление либо
  откат, но не неявная потеря записи.

### [ ] 2.3. Добавить регрессии целостности

- [ ] Покрыть rename ключа в существующий ключ.
- [ ] Покрыть добавление duplicate key в последней строке.
- [ ] Покрыть duplicate hidden key без раскрытия или потери secret value.
- [ ] Покрыть invalid rename с сохранением порядка, значения и hidden-флага.
- [ ] Покрыть cancel/close диалога и явно закрепить, какие изменения должны сохраняться.
