...
| [`TabsPresenter`](file:///home/src/pypost/ui/presenters/tabs_presenter.py) | `pypost/ui/presenters/tabs_presenter.py` | Request, WebSocket, and MCP Client (`McpClientTab`) tab lifecycle (open, close, dirty state, save, `_request_tab_count`); blank-tab protocol picker (`handle_new_tab` → `open_blank_tab`); registry-gated WS draft persist and Discard/Keep dirty-close (`tabs_presenter_draft`); duck-typed `close_tab` → `presenter.teardown()`; request execution delegation to worker threads. See [new_tab_protocol_picker.md](new_tab_protocol_picker.md), [websocket_draft_tab.md](websocket_draft_tab.md), and [mcp_client_draft_tab.md](mcp_client_draft_tab.md). |
| [`McpClientPresenter`](file:///home/src/pypost/ui/presenters/mcp_client_presenter.py) | `pypost/ui/presenters/mcp_client_presenter.py` | MCP Client draft Connect / Refresh / Invoke / Disconnect / teardown; GUI-thread `resolve_outbound_fields`; worker `run(..., headers=)` for `list_tools` and `call_tool` (`kind=invoke`); `select_tool` schema bind; Connect vs Refresh vs failed-invoke chrome. See [mcp_client_draft_tab.md](mcp_client_draft_tab.md). |
