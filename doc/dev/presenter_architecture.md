...
| [`McpClientPresenter`](file:///home/src/pypost/ui/presenters/mcp_client_presenter.py) | `pypost/ui/presenters/mcp_client_presenter.py` | MCP Client draft Connect / Disconnect / teardown (local chrome); env snapshot; `resolve_outbound_fields` / `execute_outbound` → `MCPClientService.run(..., headers=)`. Live Connect UI is MCP-TM-3 and must call `execute_outbound`. See [mcp_client_draft_tab.md](mcp_client_draft_tab.md). |
