...
| [`McpClientPresenter`](file:///home/src/pypost/ui/presenters/mcp_client_presenter.py) | `pypost/ui/presenters/mcp_client_presenter.py` | MCP Client draft Connect / Refresh / Disconnect / teardown; GUI-thread `resolve_outbound_fields`; worker `MCPClientService.run(..., headers=)` for `list_tools`; sync `execute_outbound`; Connect vs Refresh error chrome. See [mcp_client_draft_tab.md](mcp_client_draft_tab.md). |
