...
Stream inspector file export from
[`WebSocketStreamView`](file:///home/src/pypost/ui/widgets/websocket/stream_view.py) runs off
the GUI thread (PYPOST-1144):

1. **GUI thread:** capture `stream.snapshot()`, drop counters, and environment snapshots
   via `_resolve_export_env()`.
2. **Worker thread:**
  [`WebSocketStreamExportWorker`](file:///home/src/pypost/core/qt/websocket_stream_export_worker.py)
   formats and writes using
