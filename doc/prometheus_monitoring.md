...
| `gui_new_tab_actions_total` | Counter | `source`, `protocol` | Completed new-tab choice. `source`: `plus_button`, `shortcut`, `collections_context`, `unknown`. `protocol`: `http`, `websocket`, `unknown`. Picker cancel does not increment. |
