...
- **Jira `jira_get_transitions` / `jira_transition_issue` permission** was denied during execution.
  All three issues were progressed in Jira via comments (kickoff through closure), but their
  statuses were not transitioned to **Done** automatically. Manual transition or permission
  grant required to close PYPOST-418, PYPOST-419, and PYPOST-420 in Jira.
