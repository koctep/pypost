...
class CurlGenerator:
    ...
    def generate(request: RequestData, variables: dict, template_service: TemplateService) -> str:
        ...
        method = template_service.render_string(request.method, variables, render_path="curl")
        url_rendered = template_service.render_string(request.url, variables, render_path="curl")
        ...
        for k, v in request.params.items():
            rendered_k = template_service.render_string(k, variables, render_path="curl")
            rendered_v = template_service.render_string(v, variables, render_path="curl")
            if rendered_k:
