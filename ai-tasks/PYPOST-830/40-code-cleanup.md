# PYPOST-830: Code Cleanup Report
...
- `tests/test_collection_storage_gateway.py` — uses
  `@pytest.mark.usefixtures("qapp")`; no local `QApplication` /
  `setUpClass`; `pytestmark = pytest.mark.timeout(120)` present
- `tests/test_environment_storage_gateway.py` — same shared-fixture pattern;
  imports and helpers intact; no dead code
- `tests/test_storage_gateway_h3_stress.py` — stress harness aligned on
  shared `qapp`; no print/pdb; timeout marker present
...
Commands:
- `make lint` — exit 0
- `flake8 --max-line-length=100` on the three scoped gateway test modules —
  exit 0 (no line >100)
- `make test PYTEST_ARGS="tests/test_collection_storage_gateway.py
  tests/test_environment_storage_gateway.py
  tests/test_storage_gateway_h3_stress.py
  tests/test_env_storage_responsiveness.py -q"` — **20 passed** in ~14s

Timeout markers:
- `pytestmark = pytest.mark.timeout(120)` in all three scoped modules
- Existing markers retained in `tests/test_env_storage_responsiveness.py`
...
cleanup (same pattern as PYPOST-829). Ready for Step 5 (Observability).
