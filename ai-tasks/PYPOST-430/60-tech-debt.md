...
| --- | --- | --- | --- |
| TD-1 | Low | Dual SSE + Streamable HTTP maintenance | Legacy `/sse` mounts (PYPOST-551 TD-1). |
| TD-2 | Low | Broad HTTPClient content-type handling | [PYPOST-203](https://pypost.atlassian.net/browse/PYPOST-203) |
| TD-3 | Low | SSE probe without explicit Accept uses default timeout until headers | Acceptable; probe timeouts apply when user sets Accept |
