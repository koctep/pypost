# PYPOST-884: Observability Implementation

## Scope

**TEST-HARNESS ONLY.** This task aligned
`tests/test_collection_storage_worker.py` onto the shared suite `qapp` fixture
(`@pytest.mark.usefixtures("qapp")`). **No production product code was
...
| Area | Assessment |
| --- | --- |
| Key production components | Unchanged (`CollectionStorageWorker`) |
| Critical path under test | Worker load finished / failed signals |
...
2. Worker tests exercise load finished/failed via existing helpers.
3. On timeout: enriched `AssertionError` from `process_until` (prior tickets).
4. Alignment guard fails CI if the module regresses to module-local lifecycle.
...
1. Requirements and architecture confine the change to test harness consistency;
   product worker runtime behavior is intentionally unchanged.
...
3. Replacing module-local `setUpClass` `QApplication` with shared `usefixtures`
   does not alter load outcomes or error paths.
...
- [x] Logging works in error scenarios (existing worker ERROR path unchanged)
- [x] Large data structures are not logged (no new logs)
- [x] Metrics are available for monitoring (N/A)

## Notes

Observability for this ticket is the persistent AST alignment guard plus
existing `process_until` timeout detail — not production syslog/metrics.
