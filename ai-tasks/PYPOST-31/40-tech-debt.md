...
- [FIXED in PYPOST-33] In `MainWindow.on_request_finished` and `on_request_error`, worker cleanup logic is duplicated. In the future, with more complex request completion logic, this could be extracted into a separate `_cleanup_request(tab)` method.
