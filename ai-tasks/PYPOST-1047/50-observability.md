# PYPOST-1047: Observability Implementation

## Verdict

**N/A — fixtures, docs, and contract tests only.** This story adds
`jira-delete-sprint` to the curated jira-mcp collection, locks and documents
`jira-move-issues-to-backlog` as the remove-from-sprint path, and updates
companion README / testing docs plus offline fixture contracts. No application
packages under `pypost/`, request send paths, MCP handlers, logging calls, or
Prometheus instruments were added or changed.
...
| Key components | Expanded static fixtures (`examples/collections/jira_mcp.json`),
  Markdown coverage/docs, extended contract tests |
...
requests exist (`jira_delete_sprint` plus the already-exposed backlog move);
this story does not add series, log lines, or dashboards.
...
This story does not wrap or duplicate those paths. New and locked MCP-exposed
...
fixture import success rates, example adoption, delete-sprint call counts, or
per-request counters in the curated collection.
...
user-driven traffic (including delete-sprint and move-to-backlog); those are
pre-existing instruments, not new ones from this story.
...
- [x] Extended contract tests in `tests/test_example_fixtures.py` (count
  floor ≥22, `jira-delete-sprint` DELETE, `jira-move-issues-to-backlog`
  POST, placeholder hygiene) are CI/dev checks only — not production
  metrics or log sources

## Notes

Step 6 is complete with an explicit N/A: observability work applies to
application runtime changes. PYPOST-1047 expands curated examples for sprint
delete and discoverable membership removal; it does not modify the production
